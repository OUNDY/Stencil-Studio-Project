        ...
        <div className="h-16 flex items-center justify-between px-5 border-b border-border">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center">
              <span className="text-primary-foreground font-serif text-sm">S</span>
            </div>
            <span className="font-serif text-lg">Stencil Admin</span>
          </Link>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 hover:bg-accent rounded-lg">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {sidebarItems.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className="px-1.5 py-0.5 bg-destructive text-destructive-foreground text-[10px] rounded-full">
                  {item.badge}
                </span>
              )}
            </button>
        ...
        <header className="sticky top-0 z-30 h-16 bg-background/90 backdrop-blur-lg border-b border-border flex items-center justify-between px-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                type="text"
                placeholder="Ara..."
                className="pl-10 pr-4 py-2 w-64 rounded-xl border border-border bg-muted/50 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
              />
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
              <span className="text-primary text-xs font-medium">A</span>
            </div>
          </div>
        </header>
        ...
        <main className="p-4 lg:p-8">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
              ...
              <div className="bg-card border border-border rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-border">
                  <h2 className="font-medium text-sm">En Çok Satan Ürünler</h2>
                </div>
                <div className="p-4 space-y-4">
                  {topProducts.map((product, i) => (
                    <div key={product.name} className="flex items-center gap-3">
                      <span className="w-6 h-6 bg-muted rounded-full flex items-center justify-center text-xs text-muted-foreground font-medium">
                        {i + 1}
                      </span>
                      <div className="flex-1 min-w-0">
            ...
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { icon: Package, label: "Ürün Ekle", color: "bg-primary/10 text-primary" },
                { icon: Tag, label: "Kampanya Oluştur", color: "bg-green-100 text-green-700" },
                { icon: FileText, label: "Rapor İndir", color: "bg-blue-100 text-blue-700" },
                { icon: MessageSquare, label: "Mesajları Gör", color: "bg-amber-100 text-amber-700" },
              ].map((action) => (
                <button
                  key={action.label}
                  className="bg-card border border-border rounded-2xl p-4 flex flex-col items-center gap-2 hover:shadow-organic transition-shadow"
                >
